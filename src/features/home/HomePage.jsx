  ...
  const reasons = [
    {
      ...
      desc: "Terdaftar resmi dan berkomitmen memberikan layanan amanah untuk seluruh jamaah.",
    },
    {
      icon: "SV",
      title: "Layanan Terbaik",
      desc: "Bantuan end-to-end: dokumen, visa, manasik, hingga pendampingan selama perjalanan.",
    },
    {
      icon: "EZ",
      title: "Pemesanan Mudah",
      desc: "Proses pendaftaran ringkas, respons cepat, dan pilihan pembayaran yang fleksibel.",
    },
  ];
  const galleryPlaceholders = [
    
    {
      title: "Rombongan Madinah",
      image: img2,
    },
    {
      title: "Ziarah Kebun Kurma",
      image: img3,
    },
    {
      title: "Masjid Nabawi",
        ...
        <section className="w-full bg-white/70 px-6 py-14 xl:px-12">
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {posters.slice(0, 6).map((poster) => (
              <article key={poster.id} className="overflow-hidden rounded-2xl bg-white p-2 shadow-soft">
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {services.map((service) => (
            <article key={service.title} className="rounded-2xl border border-navy/10 bg-white p-5 text-center shadow-soft">
              <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-[#1774cc] text-xs font-bold tracking-wide text-white">{service.icon}</div>
      ...
      <section className="w-full px-6 py-14 xl:px-12">
        ...
        <div className="columns-1 gap-4 space-y-4 md:columns-2 lg:columns-3">
          {galleryPlaceholders.map((item) => (
            <article
              key={item.title}
              className="mb-4 break-inside-avoid overflow-hidden rounded-2xl border border-navy/10 bg-white p-2 shadow-soft"
            >
              <img src={item.image} alt={item.title} className="h-auto w-full rounded-xl object-contain" />
            </article>
          ))}
        </div>
      </section>
